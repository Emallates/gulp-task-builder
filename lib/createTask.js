...
function buildTask(task) {
	return function(){
		...
		if(task.order && isArray(task.order)){
			...
			if(task.preLog) gulpTask = plugins.log(gulpTask, task.preLog, task);
			...
			if(task.compress && !isObject(task.compress) ) gulpTask = plugins.compress(gulpTask, task.compress, task);
